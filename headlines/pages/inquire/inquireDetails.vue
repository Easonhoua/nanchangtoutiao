<template>
	<view class="inquiredetails">
		...
		<view class="bg" style="margin-top: 150upx;">
			<image class="img" src="../../static/img/home/wz_bg@3x.png" mode="widthFix"></image>
			<view class="main" style="width: 100%;">
				<view class="inner">
					<view class="message-btn" @click="toReleaseMessage()">
						<image class="btn" src="../../static/img/home/wz_btn@3x.png"></image>
					</view>
					<view class="listbox">
						<view class="list" v-for="(item,index) in dataList" :key="index"  @click="toDetail(item)">
							<view class="top">
								<text class="txt">留言回复</text>
		...
		methods: {
			...
			toReleaseMessage() {
				if(this.request.alreadyLogin()){
					uni.navigateTo({
						url: '/pages/inquire/replyMessage?type=10' 
					})
				}
			},
			goBack() {
